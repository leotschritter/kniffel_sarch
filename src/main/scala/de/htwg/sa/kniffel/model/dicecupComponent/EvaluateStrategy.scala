...
object EvaluateStrategy {
  ...
  def sum(data: List[Int], exp: Boolean): Int = if (exp) data.sum else 0
  ...
  def threeOfAKind(data: List[Int]): Int = sum(data, mapToFrequency(data).max >= 3)

  def fourOfAKind(data: List[Int]): Int = sum(data, mapToFrequency(data).max >= 4)
