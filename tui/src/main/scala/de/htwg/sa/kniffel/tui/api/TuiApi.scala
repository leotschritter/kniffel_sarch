...
class TuiApi(using val tui: TUI):
  ...
  private val tuiFlow: Flow[HttpRequest, String, NotUsed] = Flow.fromGraph(GraphDSL.create() { implicit builder: Builder[NotUsed] =>
    ...
    val postRequestFlow = Flow[HttpRequest].mapAsync(1) { req =>
      req.uri.path.toString match {
        case "/tui/quit" =>
          Future.successful(HttpResponse(entity = tui.update("quit")))
        case "/tui/save" =>
          Future.successful(HttpResponse(entity = tui.update("save")))
        case "/tui/load" =>
          Future.successful(HttpResponse(entity = tui.update("load")))
        case "/tui/move" =>
          Future.successful(HttpResponse(entity = tui.update("move")))
        case _ =>
  ...
  Http().newServerAt("localhost", 9005).bind(
    pathPrefix("tui") {
      extractRequest { request =>
        complete(
          Source.single(request).via(tuiFlow).runWith(Sink.head).map(resp => resp)
        )
      }
    }
  )